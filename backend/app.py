...
@handle_errors
def create_menu():
    # DÜZELTİLDİ: user_email alınıyor; akilli_menu_olustur kendi içinde
    # envanteri çekiyor — ayrıca ai_icin_malzeme_listesi_hazirla çağrısına gerek yok.
    user_email = _email_from_request()
    if not user_email:
        return jsonify({"success": False, "error": "Email gerekli"}), 400

    menu = akilli_menu_olustur(user_email)
    malzemeler = ai_icin_malzeme_listesi_hazirla(user_email)
    return jsonify({
        "success": True,
        "menu": menu,
        "kullanilan_malzemeler": malzemeler[:10]
    }), 200
