...
def get_recipes_from_gemini(malzemeler_metni):
    ...
    Her menüde başlangıç, ana yemek ve tatlı olmalıdır.
    Her menü için 'eksik_malzemeler', 'zorluk' (Kolay/Orta/Zor) ve 'hazirlik_suresi' ekle.
...
def kalori_hesapla(yemek_adi):
    ...
    try:
        ...
        params = {"title": ingilizce, "apiKey": SPOONACULAR_API_KEY}
        res    = requests.get(url, params=params)
        if res.status_code == 200:
            val = res.json().get('calories', {}).get('value', 0)
            if val > 0:
                return f"{int(val)} kcal"
    except Exception as e:
        print(f"⚠️ Spoonacular kalori hatası: {e}")
 
    return gemini_kalori_tahmini(yemek_adi)

# ─── Akıllı menü ──────────────────────────────────────────────────────────────
# DÜZELTİLDİ: user_email parametresi eklendi; artık malzeme listesini
# kendisi çekiyor (eski kodda app.py ayrıca çekip gönderiyordu).
def akilli_menu_olustur(user_email):
